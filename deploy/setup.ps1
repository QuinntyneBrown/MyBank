param (
    $resourceBaseName="myBank$( Get-Random -Maximum 1000)",
    $location='eastus'
)
...
dotnet build ..\src\MyBank.Api\MyBank.Api.csproj

Write-Host 'Packaging API project for deployment' -ForegroundColor Green
dotnet publish ..\src\MyBank.Api\MyBank.Api.csproj --self-contained -r win-x86 -o ..\publish
